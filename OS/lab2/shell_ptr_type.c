...
void setup(char inputBuffer[], IntPtr args[], int *background)
{
...
int main(void)
{
   ...
   /* The following is an array of character pointers (or pointers
    *    to C-strings) that is filled in by the setup() function and
    *    then passed to execvp().  With a maximum of 80 characters for a 
    *    comand line, a command line like:  a b c d e f g h i ... could 
    *    only consist of 40 separte C-strings.
    */
   IntPtr args[MAX_LINE/2];   /* command line (of 80) has max of 40 arguments */
   int i = 0;
   ...
   while (1)                   /* Program terminates normally inside setup */
   {
      ...
      /* The following code prints the command and arguments entered at
       *   the command line. */
      printf("  The command and its arguments were:\n");
      for ( i = 0; args[i] != NULL; i++ )
      {
         printf("Element args[%d] contains a pointer of value: %p\n",
                                                                i, args[i] );
         printf("The first character that the pointer points to is: %c\n",
                                                                *args[i] );
         printf("The c-string that starts there is: %s\n", args[i] );
      }
      printf("Element args[%d] contains a pointer of value: %p\n", i, args[i] );
      printf("\n");
