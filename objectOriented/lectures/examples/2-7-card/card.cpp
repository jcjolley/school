...
bool operator >= (const Card & lhs, const Card & rhs)
{
   ...
   unsigned char iLHS = lhs.value % 13;
...
{
   int iSuit = lhs.value / 13;
   int iRank = lhs.value % 13 + input;
   if (iRank > LAST_RANK)
      iRank = LAST_RANK;
   if (iRank < FIRST_RANK)
      iRank = FIRST_RANK;

   lhs.value = ((iSuit * 13) + iRank);
...
Card operator ++ (Card & lhs, int postfix)
{
   ...
   unsigned char valueOld = lhs.value;
