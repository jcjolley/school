...
int main()
{
   DateShort dateShort;      // we can only instantiate an object of type
   DateLong  dateLong;       //   DateShort or DateLong becaus Date is abstract
   Date *    dates[2];       // we can have an array of pointers to Dates
   ...
   dateLong.copy(dateShort);
   dates[0] = &dateShort;    // we can assign each object to a pointer to
   dates[1] = &dateLong;     //    the base.  It is legal to work this way

   // Finally, display the output
   cout << "Short version:                 \"" << *dates[0] << "\"\n";
   cout << "Long version:                  \"" << *dates[1] << "\"\n";
